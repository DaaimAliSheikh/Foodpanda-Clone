...
/// ADD A copy of RESOURCES FOLDER INTO EXECUTABLE IN THE END

using namespace std;
using namespace sf;

/// write "MinGW32-make" in terminal to execute
/// Update the makefile whenever you make a new header file

/// Don't change location of FILES
/// Don't interfere with src folder
...
public:
    ...
    double calculatetotal(){
        ...
        if(coupon == "PANDA")return (subtotal1 + subtotal2 + subtotal3 + delivery)*(1.0-discount);
    ...
    void setcoupon(string coupon){
        ...
        if (coupon == "PANDA")
        {
            couponstatus = "Coupon code successfully applied!";
        }
        else
        {
...
public:
    ...
    /// Constructor for icecream shake to inherit, without shake's own properties
    // Shake(string name, float price, int servingsize, string vegetarian, string halal, string flavour, string dir) : FoodItem(name, price, servingsize, vegetarian, halal, dir), flavour(flavour) {}
...
public:
    ...
    /// updated getter setters to access the properties only through Shake class
...
int main()
{
    ...
    Desi desipoint("Desi Point", "Bahadurabad", "Mon-Sat", "12pm to 12am", 0.1, 60, 4, "Resources/desipointposter.png");
    desipoint.additem(&koilakarahi);
    desipoint.additem(&reshmi);
    desipoint.additem(&sindhibiryani);

    Burger zinger("Zinger Burger", 700, 1, "Non-Vegetarian", "Halal", "Meat: Chicken, ", "Bun: Brioche bun, ", "Sauce: Thousand Islands Sauce, ", "Resources/burger.png");
    Pizza fajita("Fajita Pizza", 1200, 5, "Non-Vegetarian", "Halal", "Toppings: Mushrooms, ", "Flavour: Fajita, ", "Stuffed Crust", "Resources/pizza.png");
    Fries fries("French Fries", 100, 1, "Vegetarian", "Halal", "Light salt, ", "Ketchup, ", "Size: Small", "Resources/fries.jpg");

    Fastfood texaschicken("Texas Chicken", "Five Star Chowrangi", "Mon - Sun", "11am to 1am", 0.1, 100, 4, "Resources/texaschickenposter.png");
    ...
    Dessert shakeshack("Shake Shack", "DHA, Phase V", "Mon-Sat", "11am to 11pm", 0.1, 120, 5, "Resources/shakeshackposter.png");
    shakeshack.additem(&gelato);
    shakeshack.additem(&shake);
    shakeshack.additem(&icecreamshake);

    //"Resources/texaschickenposter.png"
    ...
    /// Background needed for easy relative positioning of elements
    ...
    welcome.setString(" Welcome to Foodpanda, discount on coupon code PANDA!");
    ...
    /// Create and style the GUI widgets here
    /// Vector2 takes in either x and y, or another type of Vector2 and converts, can access .x and .y
    // Default origin of sprites is top left corner
    // getGlobalbounds returns floatrect, which has top left width and height

    // RectangleShape a(Vector2f(50,50));

    // ///circle in sfml is a polygon
    // CircleShape poly(25, 4); ///radius/half-size, vertices(optional)
    // poly.setFillColor(Color(255, 212, 159, 0)); //4th is optional/opacity
    // poly.setOutlineColor(Color::Transparent); //predefined colors
    // poly.setOutlineThickness(4);

    // ///line(array of vertices)
    // Vertex line[] = {Vertex(Vector2f(bg.getPosition()), Color::Blue), Vertex(Vector2f(bg.getSize()), Color::Red)}; ///vertex accepts vector2f only

    // ///Textures
    // Texture logotex;
    // logotex.loadFromImage(logoimg);

    // Sprite logo;
    // logo.setTexture(logotex); ///sprites require textures
    // a.setTexture(&logotex);

    // 3 states, home, menu, checkout
    std::string temp;
    ...
    discount1.setString("> "+to_string(floor(texaschicken.getDiscount() * 100)).substr(0,5) + "\% off on code PANDA");
    discount2.setString("> "+to_string(floor(shakeshack.getDiscount() * 100)).substr(0,5) + "\% off on code PANDA");
    discount3.setString("> "+to_string(floor(desipoint.getDiscount() * 100)).substr(0,5) + "\% off on code PANDA");
    ...
    // Texture t4; t4.loadFromFile(gelato.getdir());
    // Texture t5; t5.loadFromFile(shake.getdir());
    // Texture t6; t6.loadFromFile(icecreamshake.getdir());
    // Texture t7; t7.loadFromFile(koilakarahi.getdir());
    // Texture t8; t8.loadFromFile(reshmi.getdir());
    // Texture t9; t9.loadFromFile(sindhibiryani.getdir());
